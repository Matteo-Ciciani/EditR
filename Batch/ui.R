...
shinyUI(
  ...
  titlePanel(list(HTML('<a href="https://twin-cities.umn.edu/"><img src=https://upload.wikimedia.org/wikipedia/commons/thumb/f/f9/Minnesota_Golden_Gophers_logo.svg/2560px-Minnesota_Golden_Gophers_logo.svg.png class="img.unframed" align="right" alt="UMN" height="95" width="170"></a>'), 
                  paste("BatchEditR", editrversion)),
             windowTitle="BatchEditR: Batch Edit Deconvolution by Inference of Traces in R"),
  ...
  sidebarPanel(
    ...
    p("Please specify the number of Sanger sequencing files:"),
    numericInput("nfiles", "number of files", value = 1, min = 1, step = 1),
    p("Please upload your Sanger sequencing files here:"),
    uiOutput("fileInputs"),
    p("Please upload your control Sanger sequencing file here"),
    fileInput(inputId = 'scramble', label = 'Upload control .ab1 File'),
    radioButtons('orientation', label= "Number bases from which end of the guide?", inline=TRUE, choiceNames = c("5'", "3'"),
        choiceValues = c(5,3)),
